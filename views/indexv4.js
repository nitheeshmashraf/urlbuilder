...
app.controller('MyCtrl', [
  ...
  function ($scope) {
    ...
    $scope.generateurls = function () {
      ...
      $scope.Launch_URL_Ter_inapp = $scope.Launch_URL_Ter_inapp.replace(
        'https',
        ''
      );
      $scope.Launch_URL_Ter_inapp = $scope.Launch_URL_Ter_inapp.replace(
    ...
    $scope.generateutmname = function () {
      ...
      $scope.campaignnamepart[7] = $scope.campaignnamepart[7].replace('-', '');
      $scope.campaignnamepart[7] = $scope.campaignnamepart[7].replace('.', '');
      if ($scope.campaignnamepart[6] == 'WEB') {
        $scope.hideutms = true;
      } else {
        $scope.hideutms = false;
        if ($scope.campaignnamepart[6] == 'INAPP') {
          $scope.hideinapputms = false;
        } else {
          $scope.hideinapputms = true;
        }
      }
